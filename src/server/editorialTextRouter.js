...
- Treat team + football identity as the DEFAULT way to introduce a named player when the packet verifies both pieces. Do not make a bare surname the first meaningful mention when a verified team and position/role are available.
- Strong first-mention patterns include "Cincinnati quarterback Jones threw for...", "The Bearcats' wide receiver Smith caught...", "Cincinnati's signal-caller, Jones, accounted for...", "Hawaii running back Brown ran for...", or "Cincinnati's captain at quarterback, Jones, threw for..." when every descriptive element is explicitly verified.
...
- After the first introduction, surname remains useful shorthand, but do not let the prose collapse into repeated bare surnames. If a central player is mentioned several times and a verified contextual construction is available, do not use more than two consecutive bare-surname references before naturally re-identifying him through team + position/role, team nickname + position/role, or another verified football description.
- Stat sentences are especially good places to use team-affiliated identity. Prefer "Cincinnati quarterback Jones threw for 280 yards" over "Jones threw for 280 yards" when the position is verified and the sentence is introducing or re-anchoring that player. Prefer "The Bearcats' wide receiver Smith caught six passes" over a detached surname when the nickname and position are verified.
- When two or more same-team players are being summarized together, group them naturally instead of writing a sequence of disconnected surname clauses. If both exact positions are verified, patterns such as "Cincy running backs Smith and Brown combined for..." or "Cincinnati receivers Smith and Davis accounted for..." are encouraged. If exact roster positions are not verified, use a directly supported statistical grouping such as "Cincinnati's two leading rushers, Smith and Brown" only when the supplied numbers actually establish that distinction.
- Do not force a player descriptor into every sentence. A realistic rhythm is: contextual football introduction, surname shorthand, team/position re-anchor at the next stat or paragraph pivot, surname shorthand again.
...
- Team-affiliated constructions are strongly preferred when BOTH the team affiliation and the football role/position are verified. Natural patterns include "Cincinnati quarterback Jones," "Cincinnati's quarterback, Jones," "Cincinnati's signal-caller," "Hawaii running back Smith," "the Bearcats' wide receiver Brown," or an appositive such as "Cincinnati's signal-caller, Jones." These are style patterns, not permission to invent the underlying team or position.
...
- For Cincinnati local coverage and conversational podcast copy, "Cincy" is an approved occasional shorthand, so phrases such as "Cincy's quarterback", "Cincy's signal-caller, Jones", "Cincy's senior running back, Johnson", or "Cincy running backs Smith and Brown" may be used sparingly when every descriptive detail is verified. Prefer Cincinnati or the verified team identity in more formal regional/national copy. Do not invent informal shorthand for other schools unless it is explicitly supplied.
...
- Captaincy is never stylistic filler. Use "captain" only when that exact player's captain status is explicitly verified for the relevant historical week. When verified, natural forms include "Cincinnati captain and quarterback Jones" or "Cincinnati's captain at quarterback, Jones" rather than inventing a leadership label from prominence or statistics.
