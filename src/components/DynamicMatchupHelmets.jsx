...
const Helmet = ({ brand, side, teamName }) => {
  ...
  const decalX = side === 'right' ? 111 : 96;
  ...
  return (
    ...
    >
      <g transform={mirror}>
        <defs>
          <linearGradient id={`helmet-shine-${side}`} x1="0" y1="0" x2="1" y2="1">
        ...
        <path
          ...
          fill={`url(#helmet-shine-${side})`}
          clipPath={`url(#shell-clip-${side})`}
        />
...
const useBrand = (teamName, highSchool, overrides = {}) => {
  const fallback = useMemo(
    () => fallbackTeamBrand(teamName, { ...overrides, source: highSchool ? 'high-school' : 'fallback' }),
