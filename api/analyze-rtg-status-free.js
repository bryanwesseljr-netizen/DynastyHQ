...
const SCREEN_TYPES = [
  'rtg_overview', 'rtg_academics', 'rtg_leadership',
  'rtg_health', 'rtg_fitness', 'rtg_brand', 'unknown',
];
...
export default async function handler(req, res) {
  ...
  try {
    const result = await analyzeVisionFreeFirst({
      ...
      userText: `Analyze this RTG Weekly Agenda screenshot (${String(fileName || 'upload').slice(0, 160)}). Tracked player context: ${buildPlayerContext(player)}`,
      imageDataUrl,
    ...
    console.error('Free-first RTG screenshot analysis failed', error);
    const status = Number(error?.status) === 429 ? 429 : 502;
    return json(res, status, {
      error: status === 429
        ? 'RTG screenshot analysis is out of available AI quota right now. Try again later.'
