  ...
  return (
    <div className="relative z-10 mx-auto max-w-[1500px] pb-10 animate-in fade-in">
      <section className="dhq-command-hero relative left-1/2 w-screen -translate-x-1/2 overflow-hidden border-b border-white/10 bg-[#03090d]">
        ...
        <img src={wesselPlayerHero} alt="Wessel, number 2, facing the stadium field" className="dhq-hero-player pointer-events-none absolute z-[1] hidden object-contain drop-shadow-[0_30px_36px_rgba(0,0,0,.95)] lg:block" />

        <div className="dhq-command-hero-inner relative z-[2] mx-auto flex max-w-[1500px] flex-col px-5 py-9 sm:px-8 lg:px-10">
          <div className="dhq-hero-copy max-w-[620px] lg:w-[47%]">
          ...
          <blockquote className="dhq-hero-quote mt-8 w-full max-w-[330px] self-end rounded-lg border border-white/20 bg-[#071017]/82 px-6 py-5 shadow-2xl backdrop-blur-md lg:absolute lg:mt-0 lg:w-[24%]">
      ...
      <div className="dhq-dashboard-grid -mt-px grid gap-3 px-3 pt-3 sm:px-5 lg:grid-cols-12 lg:px-8">
        <DashboardCard title="Current Phase" className="lg:col-span-4">
          <div className="relative min-h-[235px] overflow-hidden p-5">
        ...
        <DashboardCard title="Season Snapshot" actionLabel="Full team stats" onAction={() => onNavigate('dataEntry')} className="lg:col-span-5">
          <div className="grid grid-cols-2 gap-2 p-4 sm:grid-cols-4">
            {snapshotMetrics.slice(0, 8).map((metric) => (
              <div key={metric.label} className="flex min-h-[80px] flex-col items-center justify-center rounded border border-white/[0.06] bg-black/20 px-2 py-3 text-center">
        ...
        <DashboardCard title={isCoach ? 'Road to Glory Career' : 'Road to Glory'} actionLabel={isCoach ? 'Complete' : 'Active'} className="lg:col-span-4" >
          <div id="road-to-glory-snapshot" className="min-h-[235px] p-5">
        ...
        <DashboardCard title={isCoach ? 'Dynasty Central' : 'Journey Central'} actionLabel="Open hub" onAction={() => onNavigate(isCoach ? 'frontOffice' : 'dataEntry')} className="lg:col-span-4">
          <div id="dynasty-central-snapshot" className="min-h-[235px] divide-y divide-white/[0.06] px-4 py-2">
        ...
        <DashboardCard title="Newsroom" actionLabel="View all" onAction={() => onNavigate('newsroom')} className="lg:col-span-6">
          {leadArticle ? (
            <div className="grid min-h-[235px] gap-4 p-4 sm:grid-cols-[1.1fr_.9fr]">
      ...
      <footer className="mx-3 mt-3 flex flex-col items-center justify-between gap-3 border-t border-white/[0.07] px-3 py-6 text-center sm:mx-5 sm:flex-row sm:text-left lg:mx-8">
        <div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full border border-slate-700 text-center text-[5px] font-black italic leading-[0.82] text-slate-400">EA<br />SPORTS</span><span className="text-[8px] font-black uppercase tracking-wider text-slate-400">College Football 27 Command Center</span></div>
