...
const NewsroomMediaManager = ({
  ...
  const receiveFile = (event, asReference) => {
    ...
    event.target.value = '';
    if (file) onUpload(file, { issue, article, asReference });
  };
    ...
    <section className="border-t border-slate-700 bg-slate-950 p-4 text-slate-100">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button" disabled={busy} onClick={() => uploadRef.current?.click()} className="flex items-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-[10px] font-black uppercase tracking-wider text-white hover:bg-blue-500 disabled:opacity-50">
          {busy ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />} Add Photos to Library
        </button>
        {!lockerOnly && <button type="button" disabled={busy || !mediaLibrary.length} onClick={() => setLibraryOpen((open) => !open)} className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-[10px] font-black uppercase tracking-wider hover:border-slate-500 disabled:opacity-40">
          <Images size={14} /> Career Photo Library
        </button>}
        ...
        <button type="button" onClick={() => setLockerOpen((open) => !open)} className={`${lockerOnly ? '' : 'ml-auto'} flex items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-[10px] font-black uppercase tracking-wider text-amber-200 hover:bg-amber-500/20`}>
          <ImagePlus size={14} /> AI Reference Locker ({mediaLibrary.filter((asset) => asset.isReference).length})
        </button>
      </div>

      <p className="mt-2 text-[10px] leading-relaxed text-slate-500">{lockerOnly ? 'Build your reusable career photo library before the first article. Uploaded photos never trigger AI generation.' : 'Uploads go to your reusable library. New and rewritten editions automatically choose uploaded photos; AI images are created only when you press Generate AI Photo.'}</p>
        ...
        <div className="mt-4 rounded-xl border border-amber-500/20 bg-amber-950/10 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[10px] text-slate-500">Regular uploads can be selected automatically for articles. Mark only clear face, uniform, helmet, or equipment photos as AI references.</p>
            </div>
            <button type="button" disabled={busy} onClick={() => referenceRef.current?.click()} className="flex items-center gap-2 rounded-lg border border-amber-500/30 px-3 py-2 text-[10px] font-black uppercase tracking-wider text-amber-200 hover:bg-amber-500/10 disabled:opacity-50">
              <Upload size={13} /> Add AI Reference
            </button>
          </div>
                ...
                <div key={asset.id} className={`overflow-hidden rounded-lg border ${asset.isReference ? 'border-amber-400' : 'border-slate-700'}`}>
                  ...
                  <div className="space-y-2 p-2">
                    <label className="flex cursor-pointer items-center gap-2 text-[9px] font-bold text-slate-300">
                      <input type="checkbox" checked={Boolean(asset.isReference)} onChange={(event) => onToggleReference(asset, event.target.checked)} className="accent-amber-500" /> Approved reference
                    </label>
                    <button type="button" onClick={() => onDelete(asset)} className="flex items-center gap-1 text-[9px] font-bold text-slate-500 hover:text-red-300"><Trash2 size={11} /> Delete</button>
                  </div>
                </div>
              ))}
            </div>
          ) : <p className="mt-4 rounded-lg border border-dashed border-slate-700 p-5 text-center text-xs text-slate-500">Upload your first reusable game, action, recruiting, or portrait photo.</p>}

          <label className="mt-4 flex items-start gap-3 rounded-lg border border-slate-700 bg-slate-950/70 p-3">
            <input type="checkbox" checked={autoAssignLibrary} onChange={(event) => onSetAutoAssignLibrary(event.target.checked)} className="mt-0.5 accent-blue-500" />
            <span>
