...
- On tracked-player game-stat screens, game.passYds, game.passTD, game.rushYds, game.rushTD, and game.int are the tracked PLAYER'S stats, not team totals. When the label and value are visible, extract every visible one of these fields even when the value is 0. Never omit a player stat just because its visible value is zero.
- On postgame team-comparison screens, use game.team* for the tracked team column and game.opponent* for the opponent. Inspect every visible stat row rather than stopping after a few rows. Extract plainly labeled Total Offense, First Downs, Turnovers, Rushing Yards, and Passing Yards for both columns, including visible zeroes. If the Rushing Yards row is visible, emit both game.teamRushYds and game.opponentRushYds. Never calculate a missing rushing value from Total Offense minus Passing Yards, and never guess column ownership.
