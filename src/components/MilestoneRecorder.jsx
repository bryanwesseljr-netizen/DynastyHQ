...
const MilestoneRecorder = ({ season, week, institution, onPublish }) => {
  ...
  return (
    <section className="milestone-recorder-layout rounded-xl border border-amber-500/25 bg-slate-950/50 p-4 shadow-inner xl:grid xl:grid-cols-[minmax(140px,0.42fr)_minmax(0,1.58fr)] xl:items-start xl:gap-4">
      ...
      <div className="milestone-recorder-fields mt-4 grid grid-cols-1 gap-3 sm:grid-cols-6 xl:mt-0">
        ...
        <label className={`block ${showPreviousInstitution || showAchievement ? 'sm:col-span-3' : 'sm:col-span-6'}`}>
        ...
        <label className={`flex cursor-pointer items-start gap-3 rounded-lg border p-3 sm:col-span-4 ${draft.confirmed ? 'border-emerald-500/35 bg-emerald-950/20' : 'border-slate-700 bg-slate-900/70'}`}>
        ...
        <button type="button" onClick={submit} className="flex w-full items-center justify-center gap-2 self-stretch rounded-lg bg-amber-500 px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-950 transition-colors hover:bg-amber-400 sm:col-span-2">
