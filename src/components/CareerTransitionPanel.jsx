  ...
  if (stage === CAREER_STAGES.HIGH_SCHOOL && state.player?.isCommitted) {
    return (
      <section className="rounded-2xl border border-amber-400/40 bg-amber-950/20 p-6 shadow-2xl">
        ...
        <p className="mt-2 max-w-3xl text-sm text-slate-300">This freezes high-school recruiting as permanent history, advances to the freshman college season, and turns Recruiting into the quiet transfer hub.</p>
        {!readOnly && <button type="button" disabled={!fiveGamesComplete} onClick={onBeginCollege} className="mt-5 flex items-center gap-2 rounded-xl bg-amber-500 px-5 py-3 text-xs font-black uppercase tracking-wider text-slate-950 disabled:cursor-not-allowed disabled:bg-slate-800 disabled:text-slate-500"><Play size={15} /> {fiveGamesComplete ? 'Begin College Career' : 'Complete all five high-school games'}</button>}
      </section>
    );
  }
