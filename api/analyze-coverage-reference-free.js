...
const CATEGORIES = [
  ...
  'punting', 'scoring', 'team_note', 'other',
];
...
export default async function handler(req, res) {
  ...
  try {
    const result = await analyzeVisionFreeFirst({
      ...
      userText: `Analyze ${String(fileName || 'coverage screenshot').slice(0, 160)} as editorial-only postgame reference material. Tracked program context: ${String(school || '').slice(0, 120)}. The program context helps identify sides of a matchup but is not screenshot evidence.`,
      imageDataUrl,
    ...
    console.error('Free-first coverage reference analysis failed', error);
    const status = Number(error?.status) === 429 ? 429 : 502;
    return json(res, status, {
      error: status === 429
        ? 'Coverage analysis is out of available AI quota right now. Try again later.'
