...
const buildPulse = (state) => {
  ...
  if (stage === CAREER_STAGES.HIGH_SCHOOL) {
    ...
    items.push(
      ...
      { label: evaluations.length < 5 ? 'NEXT' : 'STATUS', value: evaluations.length < 5 ? `GAME ${evaluations.length + 1}` : 'TAPE COMPLETE' },
    );
...
const useHeaderPresence = () => {
  ...
  useEffect(() => {
    const check = () => setPresent(Boolean(document.querySelector('header.no-print')));
    check();
...
const DynastyPulse = () => {
  const headerPresent = useHeaderPresence();
